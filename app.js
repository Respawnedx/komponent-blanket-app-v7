...
async function saveCurrentRecord(options = {}){
  ...
  const changes = computeTagChanges(prevRec, rec);
  const hasTagChanges = hasMeaningfulTagChanges(changes);
  ...
  let revisionAction = "none";
  if(hasTagChanges){
    rec.revisions.push({ at: rec.updatedAt, by: user.initials, desc: revDesc, changes, source: isAuto ? "auto" : "manual" });
    revisionAction = isAuto ? "auto_revision_created" : "manual_revision_created";
  }else if(!isAuto && annotateLatestAutoRevision(rec, revDesc)){
    revisionAction = "latest_auto_revision_annotated";
  }else if(!isAuto){
    rec.revisions.push({ at: rec.updatedAt, by: user.initials, desc: revDesc, changes, source: "manual" });
    revisionAction = "manual_note_created";
  }
  ...
  try{
    ...
    await logAudit({
      ...
      record_id: rec.id,
      hovednr: rec.hovedkomponentnr || null,
      meta: { selectedCount: (rec.selectedCodes||[]).length, revision: revDesc, changes, revisionAction }
    });
...
function hasMeaningfulTagChanges(changes){
  const text = String(changes || "").trim();
  return !!text && text !== "Ingen tag-ændringer.";
}
...
function latestRevisionIndex(revisions){
  if(!Array.isArray(revisions) || !revisions.length) return -1;
  let bestIdx = 0;
  let bestAt = String(revisions[0]?.at || "");
  for(let i=1; i<revisions.length; i++){
    const at = String(revisions[i]?.at || "");
    if(at.localeCompare(bestAt) > 0){
      bestAt = at;
      bestIdx = i;
    }
  }
...
function isAutoGeneratedRevision(rev){
  if(!rev) return false;
  if(rev.source === "auto" && !rev.annotatedAt) return true;
  const desc = String(rev.desc || "").trim();
  const first = firstRevisionLine(rev.changes);
  return !!desc && !!first && desc === first;
}

function annotateLatestAutoRevision(rec, desc){
  const cleanDesc = String(desc || "").trim();
  if(!cleanDesc || !Array.isArray(rec?.revisions) || !rec.revisions.length) return false;

  const idx = latestRevisionIndex(rec.revisions);
  if(idx < 0) return false;

  const latest = rec.revisions[idx];
  if(!hasMeaningfulTagChanges(latest?.changes)) return false;
  if(!isAutoGeneratedRevision(latest)) return false;

  rec.revisions[idx] = {
    ...latest,
    desc: cleanDesc,
    annotatedAt: rec.updatedAt,
    annotatedBy: rec.editedBy,
  };
  return true;
}
